...
BDS<-function(dat,Noise=1,Mazur=1,Exponential=1,Rachlin=1,GreenMyerson=1,BD=1){
  ...
  chartFrame<-data.frame(chart=chartFunction(X, Y, exp.frame$exp.lnk,Mazur.frame$Mazur.lnk,
                                             ...
                                             prob.frame$Noise,
                                             prob.frame$Mazur,
                                             prob.frame$Exponential,
                                             prob.frame$GreenMyerson,
                                             prob.frame$Rachlin,
                                             prob.frame$BD,
                                             lnED50.mostprob))
...
chartFunction<-function(mDelays, mValues, samuelsonK=0,ainslieK=0,betaConstant=0,deltaConstant=0,
                        ...
                        prob.Noise=0, prob.Mazur=0,
                        prob.Exponential=0, prob.GreenMyerson=0,
                        prob.Rachlin=0, prob.BD=0, ED50=0){
  ...
  mShowFrame = data.frame(legend = c(paste("Noise: ", round(prob.Noise, 5), sep = ""),
                                     paste("Exponential: ", round(prob.Exponential, 5), sep = ""),
                                     paste("Hyperbolic: ", round(prob.Mazur, 5), sep = ""), 
                                     paste("QuasiHyperbolic: ", round(prob.BD, 5), sep = ""), 
                                     paste("MyersonGreen: ", round(prob.GreenMyerson, 5), sep = ""), 
                                     paste("Rachlin: ", round(prob.Rachlin, 5), sep = "")),
                          col = c("red", 
                                  "blue", 
                                  "green", 
                                  "brown", 
