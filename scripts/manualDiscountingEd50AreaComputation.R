...
BDS<-function(dat,Noise=1,Mazur=1,Exponential=1,Rachlin=1,GreenMyerson=1,BD=1){
  ...
  if (prob.frame$probmodel == "Rachlin") {
    AUK <- integrate(integrandRachlin, lower = min(mData$Y), upper = max(mData$X), lnK = Rachlin.frame$Rachlin.lnk, s = Rachlin.frame$Rachlin.s)$value/maximumArea

  } else if (prob.frame$probmodel == "Exponential") {
    AUK <- integrate(integrandExp, lower = min(mData$Y), upper = max(mData$X), lnK = exp.frame$exp.lnk)$value/maximumArea

  } else if (prob.frame$probmodel == "Mazur") {
    AUK <- integrate(integrandHyp, lower = min(mData$Y), upper = max(mData$X), lnK = Mazur.frame$Mazur.lnk)$value/maximumArea

  } else if (prob.frame$probmodel == "GreenMyerson") {
    AUK <- integrate(integrandMyerson, lower = min(mData$Y), upper = max(mData$X), lnK = MG.frame$MG.lnk, s = MG.frame$MG.s)$value/maximumArea

  } else if (prob.frame$probmodel == "BD") {
    AUK <- integrate(integrandBetaDelta, lower = min(mData$Y), upper = max(mData$X), beta = BD.frame$BD.beta, delta = BD.frame$BD.delta)$value/maximumArea

  } else {
    AUK <- noise.frame$noise.mean
  }

  lnED50res<-data.frame(prob.frame$probmodel,lnED50.mostprob,lnED50.Mazur,Mazur.frame$Mazur.lnk, AUC, AUK)

  return(list(noise.frame,Mazur.frame,exp.frame,MG.frame,Rachlin.frame,prob.frame,startstore,lnED50res,BD.frame))
}


#
# Area output example: BDS(dataFrame,1,1,1,1,1,1)
#

dataFrame <- data.frame(X=c(1,30,180,360,720,1440),
                        Y=c(1,0.9,0.8,0.7,0.6,0.5))

dataFrame$ses <- 1

BDS(dataFrame,1,1,1,1,1,1)